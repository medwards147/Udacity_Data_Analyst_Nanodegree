# -*- coding: utf-8 -*-
"""
Created on Thu Nov 12 21:04:02 2015
...
# Parking amenity by city

#    pipeline.append( {"$match": {"amenity": "restaurant" } } )
#    pipeline.append( {"$group": {"_id": "$cuisine",
#                                 "count": {"$sum" : 1 } } } )
#    pipeline.append( { "$sort" : { "count" : -1 } } )
#    pipeline.append( { "$limit" : 5 } )
#    return pipeline

    pipeline.append( {"$match": {"amenity": "address" } } )
    pipeline.append( {"$group": {"_id": "$address.postcode",
                                 "count": {"$sum" : 1 } } } )
    pipeline.append( { "$sort" : { "count" : -1 } } )
    pipeline.append( { "$limit" : 5 } )
    return pipeline
...
# total fix me fields
#    pipeline.append( { "$match" : {"fixme" : { "$exists":1 } } } )
#    #pipeline.append( { "$project" : { "count" : "$count", "fixme" :  "$fixme" } } )
#    pipeline.append( { "$group" : { "_id" : "$fixme", "count" : { "$sum": 1 } } } )
#    #pipeline.append( { "$group" : { "_id" : "$count", "total" : { "$sum": 1 } } } )
#    pipeline.append( { "$sort" : { "count" : -1 } } )
#   # pipeline.append( { "$limit" : 5 } )
#    return pipeline
